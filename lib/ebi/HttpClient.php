...
class HttpClient{
	...
	private function build_query(array $vars): string {
	...
	private function apply_base_options(\CurlHandle $ch, string $url, string $method): string{
		...
		curl_setopt($ch, CURLOPT_MAXREDIRS, $this->redirect_max);
		...
		switch ($method) {
			case 'POST':
				if (!empty($this->request_file_vars)) {
					...
					if (!empty($this->request_vars)) {
						foreach (explode('&', $this->build_query($this->request_vars)) as $q) {
							$s = explode('=', $q, 2);
							$vars[urldecode($s[0])] = $s[1] ?? null;
						}
					}
					foreach (explode('&', $this->build_query($this->request_file_vars)) as $q) {
						$s = explode('=', $q, 2);
						...
						if (isset($s[1])) {
							...
							$vars[urldecode($s[0])] = (class_exists('\\CURLFile', false))
								? new \CURLFile($f, null, basename($f))
								: '@'.$f;
						}
					}
					curl_setopt($ch, CURLOPT_POSTFIELDS, $vars);
				} else {
					curl_setopt($ch, CURLOPT_POSTFIELDS, $this->build_query($this->request_vars));
				}
				break;
			...
			case 'HEAD':
				if (!empty($this->request_vars)) {
					$url .= '?'.$this->build_query($this->request_vars);
				}
				break;
		}
		...
		return $cookie_base_domain;
	}

	/**
	 * 指定したURLのファイルサイズを取得する
	 */
	public function content_length_download(string $url): int{
		$ch = curl_init();
		$this->apply_base_options($ch, $url, 'HEAD');
	...
	private function parallel_download(string $url, int $size, string $download_path, int $parallel = 4): void {
		$chunk_size = (int)ceil($size / $parallel);
		$mh = curl_multi_init();
		$chs = [];
		$temp_files = [];

		if (!is_dir(dirname($download_path))) {
		...
		try {
			for ($i = 0; $i < $parallel; $i++) {
				$start = $i * $chunk_size;
				$end   = min($size - 1, ($i + 1) * $chunk_size - 1);

				$tmp = sprintf('%s.part.%d', $download_path, $i);
				$temp_files[] = $tmp;
				...
				$ch = curl_init();
				$this->apply_base_options($ch, $url, 'GET');
			...
			do {
				...
				if ($status > 0) {
					throw new \RuntimeException("cURL error: " . curl_multi_strerror($status));
				}
			...
			// 結合
			$out = fopen($download_path, 'wb');
			if ($out === false) {
				throw new \RuntimeException("Failed to open output file: $download_path");
			}
			foreach ($chs as [$ch, $fpw, $tmp]) {
				// 転送が失敗してないかチェック
				if (curl_errno($ch) !== 0) {
				...
				$fpr = fopen($tmp, 'rb');
				if ($fpr === false) {
					throw new \RuntimeException("Failed to open temp file for reading: $tmp");
				}

				stream_copy_to_stream($fpr, $out);
				fclose($fpr);
				unlink($tmp);
	...
	private function request(string $method, string $url, ?string $download_path=null): self{
		...
		$cookie_base_domain = $this->apply_base_options($ch, $url, $method);
		...
		if(empty($download_path)){
			...
			curl_setopt($ch, CURLOPT_BUFFERSIZE, 1024 * 128);
		...
		try{
			...
			if(($err_code = curl_errno($ch)) > 0){
				if($err_code == 47 || $err_code == 52){
					return $this;
				}
				throw new \ebi\exception\ConnectionException($err_code.': '.curl_error($ch).' ('.$url.')');
			}
			$this->url = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
			$this->status = curl_getinfo($ch, CURLINFO_HTTP_CODE);

			if(self::$recording_request){
				self::$record_request[] = curl_getinfo($ch, CURLINFO_HEADER_OUT);
			}
