...
namespace ebi;
/** 
 * Appに関連するテンプレート用のヘルパー関数群
 */
class AppHelper{
	private ?string $name = null;
	private ?\ebi\app\Request $req = null;

	public function __construct(?string $name=null, ?\ebi\app\Request $obj=null){
		$this->name = $name;
		$this->req = $obj;
	}
	...
	public function map_url(string $map_name, ...$args): string{
		if(!isset(\ebi\App::url_pattern()[$map_name])){
			if(isset(\ebi\App::url_pattern()[$map_name.'/index'])){
				$map_name = $map_name.'/index';
			}
		}
		if(isset(\ebi\App::url_pattern()[$map_name][sizeof($args)])){
			return vsprintf(\ebi\App::url_pattern()[$map_name][sizeof($args)],$args);
		}
		return '';
	}
	...
	public function package_method_url(string $name, ...$args): string{
		if(isset(\ebi\App::selected_class_pattern()[$name][sizeof($args)])){
			return vsprintf(\ebi\App::selected_class_pattern()[$name][sizeof($args)]['format'],$args);
		}
		return '';
	}
	/**
	...
	public function media(?string $path=null): string{
		return \ebi\Util::path_absolute(\ebi\App::media_url(), $path);
	}
	/**
	 * アプリケーションのURLを返す
	 */
	public function app_url(?string $path=null): string{
		return \ebi\Util::path_absolute(\ebi\App::app_url(), $path);
	}
	/**
	 * ゼロを桁数分前に埋める
	 */
	...
	public function has_invalid(?string $group=null): bool{
		return \ebi\AppInvalid::has($group);
	}
	/**
	 * 引数が空ではないか
	 ...
	 * HTMLエンコードする
	 */
	public function htmlencode(?string $value): string{
