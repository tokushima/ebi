...
class App{
	...
	private static function template(array $vars, array $selected_pattern, $ins, string $path, ?string $media, $template_dir): void{
		...
		self::$template->vars('t', new \ebi\AppHelper($selected_pattern['name'] ?? null,($ins instanceof \ebi\app\Request) ? $ins : null));
	 ...
	 * アプリケーションを実行する
	 */
	public static function run(array $map=[]): void{
		if(empty($map)){
			...
			if(preg_match('/^'.(empty($k) ? '' : '\/').str_replace(['\/','/','@#S'],['@#S','\/','\/'],$k).'[\/]{0,1}$/',$pathinfo,$param_arr)){
				...
				try{
					...
					if(!isset($funcs) && isset($class)){
						...
						$is_flow_request = ($ins instanceof \ebi\app\Request || is_subclass_of($ins, \ebi\app\Request::class));
					...
					if(!$accept_debug){
						...
						if($map_output === 'xml'){
							if(strpos(strtolower((string)(new \ebi\Env())->get('HTTP_ACCEPT')), 'application/json') === false){
								...
								foreach(\ebi\AppInvalid::get() as $g => $e){
